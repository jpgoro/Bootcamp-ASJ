...
SELECT TOP(5) count (o.id) AS 'cantidad', g.gender AS 'Generos'
FROM genders g , products p , order_details o
WHERE g.id = p.id_gender 
...
SELECT DISTINCT e.legajo, e.name, e.last_name, o.date 
FROM employees e
JOIN orders o ON e.id = o.id_employee
WHERE o.date 
BETWEEN '2024-01-12' AND '2024-01-15'
ORDER BY o.date DESC, e.legajo ASC;



SELECT COUNT(o.id) AS cantidad_pedidos, c.id AS id_client, c.name AS 'Nombre', c.last_name AS 'Apellido', pm.payment_method AS 'Metodos de pago'
FROM clients c
JOIN orders o ON c.id = o.id_client
JOIN payment_methods pm ON o.id_payment_method = pm.id
WHERE c.is_partner = 1
GROUP BY c.id, c.name, c.last_name, pm.payment_method
HAVING COUNT(o.id) >= 2;*/
...
SELECT name as Nombre, last_name as Apellido, 'Empleados' as Tipo
from employees
UNION
